...
public class MemberFindPasswordController {
	...
	@Autowired //member-servlet.xml에서 등록한 빈
	...
	public void send(MemberBean member, HttpServletResponse response) throws Exception {
		...
		if(bean == null) {
			pw.println("<script>");
			pw.println("alert('회원정보가 일치하지 않습니다.'); location.href='findPassword.mem'");
			pw.println("</script>");
			pw.flush();
			...
			String mailContent = member.getName()+"님의 "; //보낼 메시지
			mailContent += "새로운 비밀번호:   "+newPw; 
			mailMessage.setSubject("블루버드 비밀번호", "utf-8"); 
			mailMessage.setText(mailContent, "utf-8", "html");  
			mailMessage.addRecipient(Message.RecipientType.TO, new InternetAddress(member.getEmail())); //Message 여러가지 있음 javax 선택
			mailSender.send(mailMessage); //성공!

			pw.println("<script>");
			pw.println("alert('새로운 비밀번호를 "+member.getEmail()+" 로 전송했습니다.'); location.href='login.mem'");
			pw.println("</script>");
			pw.flush();
		}

		//findPw(response, member); //첫번째 방법 이메일 에러남..
	...
	public void sendEmail(MemberBean member, String div) throws Exception {
		...
		String hostSMTPpwd = "hxlrzokwqtwisxdn"; //서버 이메일 비번(보내는 사람 이메일 비번) X 22년 부터는 2단계 인증 비번 요
